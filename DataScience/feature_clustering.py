...
class WordClustering(object):
    ...
    def __init__(self, file_path, df_key, ngram_range, radius, key_word, stop_words, plot_ndim, join_words, pos_filter):
    ...
    def main(self):
        ...
        if self.plot_ndim == 3:
            vis.show_3D_in_2D(self.features, self.key_word, self.radius, self.pos_filter)
        
        elif self.plot_ndim == 2:
            vis.show_in_2D(self.features, self.key_word)
        
        else:
            raise Exception("Only dimensions of 2 or 3 are supported.")    
...
if __name__ == "__main__":
    ...
    parser.add_option("-d", "--plot-dim", dest="ndim", default=2, type="int",
                      help="""The number of dimensions for the cluster plot. Default is 2. If three is chosen, the 
                               3rd dimension will be a color.""")
